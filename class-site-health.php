...
if (!class_exists("rsssl_site_health")) {
	class rsssl_site_health {
		...
		public function rsssl_hsts_check( $tests ) {
			$tests['direct']['simple-ssl'] = array(
				'label' => __( 'Simple SSL HSTS test' ),
		...
		public function rsssl_hsts_test() {
			$result = array(
				'label'       => __( '301 SSL redirect enabled', 'simple-ssl' ),
				'status'      => 'good',
				'badge'       => array(
					'label' => __( 'SSL' ),
					'color' => 'blue',
				...
				'description' => sprintf(
					'<p>%s</p>',
					__( 'You have set a 301 redirect to SSL. This is important for SEO purposes', 'simple-ssl' )
				),
				'actions'     => '',
				'test'        => 'simple-ssl',
			);
			...
			if (RSSSL()->really_simple_ssl->has_301_redirect() && RSSSL()->really_simple_ssl->wp_redirect && RSSSL()->rsssl_server->uses_htaccess() && !RSSSL()->really_simple_ssl->htaccess_redirect) {
				...
				$result['label']       = __( '301 .htaccess redirect is not enabled.' , 'simple-ssl' );
				...
				$result['actions']     .= sprintf(
					...
					__( 'Enable 301 .htaccess redirect', 'simple-ssl' )
				);
			}
