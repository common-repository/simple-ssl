...
if (!class_exists('rsssl_multisite')) {
    class rsssl_multisite
    {
        ...
        private $pro_url = "https://www.simple-ssl.com/pro-multisite";
        ...
        */
        ...
        public function add_multisite_menu()
        {
            ...
            add_settings_section('rsssl_network_settings', __("Settings", "simple-ssl"), array($this, 'section_text'), $this->page_slug);

            add_settings_field('id_ssl_enabled_networkwide', __("Enable SSL", "simple-ssl"), array($this, 'get_option_enable_multisite'), $this->page_slug, 'rsssl_network_settings');
        ...
        */
        ...
        {
            _e("Below you can set the multisite options for Simple SSL", "simple-ssl");
        }
        ...
        public function get_option_enable_multisite()
        {
            ...
            <select name="rlrsssl_network_options[ssl_enabled_networkwide]">
                ...
                <option value="-1" <?php if (!$this->selected_networkwide_or_per_site) echo "selected"; ?>><?php _e("No selection was made", "simple-ssl") ?>
                    <?php } ?>
                <option value="1" <?php if ($this->ssl_enabled_networkwide) echo "selected"; ?>><?php _e("networkwide", "simple-ssl") ?>
                <option value="0" <?php if (!$this->ssl_enabled_networkwide) echo "selected"; ?>><?php _e("per site", "simple-ssl") ?>
            </select>
            <?php
            ...
            rsssl_help::this()->get_help_tip(__("Select to enable SSL networkwide or per site.", "simple-ssl"));
        }
        ...
        public function settings_tab()
        {
            if (isset($_GET['updated'])): ?>
                <div id="message" class="updated notice is-dismissible">
                    <p><?php _e('Options saved.', 'simple-ssl') ?></p></div>
            <?php endif; ?>
            <div class="wrap">
                <h1><?php _e('Simple SSL multisite options', 'simple-ssl'); ?></h1>
        ...
        public function show_notice_activate_networkwide()
        {
            ...
            if (!RSSSL()->really_simple_ssl->site_has_ssl) {
                ...
                <div id="message" class="error notice activate-ssl">
                    <p><?php _e("No SSL was detected. If you do have an SSL certificate, try to reload this page over https by clicking this link:", "simple-ssl"); ?>
                        &nbsp;<a
                                href="<?php echo $current_url ?>"><?php _e("reload over https.", "simple-ssl"); ?></a>
                        <?php _e("You can check your certificate on", "simple-ssl"); ?>&nbsp;<a target="_blank"
            ...
            if (is_main_site(get_current_blog_id()) && RSSSL()->really_simple_ssl->wpconfig_ok()) {
                ...
                <div id="message" class="updated notice activate-ssl">
                    <h1><?php _e("Choose your preferred setup", "simple-ssl"); ?></h1>
                    <?php _e("Some things can't be done automatically. Before you migrate, please check for: ", 'simple-ssl'); ?>
                    <p>
                    <ul>
                        <li><?php _e('Http references in your .css and .js files: change any http:// into //', 'simple-ssl'); ?></li>
                        <li><?php _e('Images, stylesheets or scripts from a domain without an SSL certificate: remove them or move to your own server.', 'simple-ssl'); ?></li>
                    </ul>
                    </p>
                    <?php $this->show_pro(); ?>
                    <p>
                    <form action="" method="post">
                        ...
                        <input type="submit" class='button button-primary'
                               value="<?php _e("Activate SSL networkwide", "simple-ssl"); ?>"
                               id="rsssl_do_activate_ssl_networkwide" name="rsssl_do_activate_ssl_networkwide">
                        <input type="submit" class='button button-primary'
                               value="<?php _e("Activate SSL per site", "simple-ssl"); ?>"
                               id="rsssl_do_activate_ssl_per_site" name="rsssl_do_activate_ssl_per_site">
                    </form>
                    </p>
                    <p>
                        <?php _e("Networkwide activation does not check if a site has an SSL certificate. It just migrates all sites to SSL.", "simple-ssl"); ?>
                    </p>
                </div>
                <?php
            }
        ...
        {
            ?>
            <p><?php _e('You can also let the automatic scan of the pro version handle this for you, and get premium support and increased security with HSTS included.', 'simple-ssl'); ?>
                &nbsp;<a target="_blank"
                         href="<?php echo $this->pro_url; ?>"><?php _e("Check out Simple SSL Premium", "simple-ssl"); ?></a>
            </p>
            <?php
        }
        ...
        */
        ...
        public function show_notices()
        {
            ...
            if (isset(RSSSL()->really_simple_ssl->errors["DEACTIVATE_FILE_NOT_RENAMED"])) {
                ...
                <div id="message" class="error notice is-dismissible rlrsssl-fail">
                    ...
                    <a href="options-general.php?page=rlrsssl_really_simple_ssl"><?php echo __("Check again", "simple-ssl"); ?></a>
                </div>
                <?php
            }
            ...
            if ($this->ssl_process_active()) {
                ...
                <div id="message" class="error notice is-dismissible rlrsssl-fail">
                    <p>
                        ...
                        $run_ssl_process_hook_switch_link = network_admin_url("settings.php?page=simple-ssl&action=ssl_conversion_hook_switch&token=" . $token);

                        $link_open = '<a target="_self" href="' . $run_ssl_process_hook_switch_link . '">';
                        $link_close = '</a>';
                        ?>

                        <?php printf(__("Conversion of websites %s percent complete.", "simple-ssl"), $this->get_process_completed_percentage()); ?>
                        <?php _e("You have just started enabling or disabling SSL on multiple websites at once, and this process is not completed yet. Please refresh this page to check if the process has finished. It will proceed in the background.", "simple-ssl"); ?>
                        <?php printf(__("If the conversion does not proceed after a few minutes, click %shere%s to force the conversion process.", "simple-ssl"), $link_open, $link_close); ?>
            ...
            */
            ...
            if ($this->selected_networkwide_or_per_site && !get_site_option("rsssl_success_message_shown")) {
                ...
                <div id="message" class="updated notice is-dismissible rlrsssl-multisite-success">
                    <p>
                        <?php _e("SSL activated!", "simple-ssl"); ?>&nbsp;
                        <?php
                        if ($this->ssl_enabled_networkwide)
                            _e("SSL was activated on your entire network.", "simple-ssl");
                        else
                            _e("SSL was activated per site.", "simple-ssl");
                        ?>

                        <?php _e("Don't forget to change your settings in Google Analytics and Webmaster tools.", "simple-ssl"); ?>
                        &nbsp;
                        <a target="_blank"
                           href="https://simple-ssl.com/knowledge-base/how-to-setup-google-analytics-and-google-search-consolewebmaster-tools/"><?php _e("More info.", "simple-ssl"); ?></a>
                    </p>
                </div>
                <?php
            }
            ...
            if (!$this->ssl_enabled_networkwide && $this->selected_networkwide_or_per_site && $this->is_multisite_subfolder_install()) {
                ...
                if (RSSSL()->really_simple_ssl->no_server_variable) {
                    ...
                    <div id="message" class="error notice">
                        <p>
                            <?php _e('You run a Multisite installation with subfolders, which prevents this plugin from fixing your missing server variable in the wp-config.php.', 'simple-ssl'); ?>
                            <?php _e('Because the $_SERVER["HTTPS"] variable is not set, your website may experience redirect loops.', 'simple-ssl'); ?>
                            <?php _e('Activate networkwide to fix this.', 'simple-ssl'); ?>
                        </p>
                    </div>
                    <?php
                }
            ...
            if (!RSSSL()->really_simple_ssl->ssl_enabled && !$this->is_multisite_subfolder_install() && !RSSSL()->rsssl_certificate->is_wildcard() && !get_site_option("rsssl_wildcard_message_shown")) {
                ...
                <div id="message" class="error notice is-dismissible rlrsssl-multisite-wildcard-warning">
                    <p>
                        <?php _e("You run a Multisite installation with subdomains, but your site doesn't have a wildcard certificate.", 'simple-ssl'); ?>
                        <?php _e("This leads to issues when activating SSL networkwide since subdomains will be forced over SSL as well while they don't have a valid certificate.", 'simple-ssl'); ?>
                        <?php _e("Activate SSL per site or install a wildcard certificate to fix this.", 'simple-ssl'); ?>
                    </p>
                </div>
                <?php
            }
                ...
                if (defined('rsssl_pro_version')) {
                    if (!defined('rsssl_pro_ms_version')) {
                        ...
                        <div id="message" class="updated notice is-dismissible rsssl-pro-dismiss-notice">
                            <p>
                                <?php echo sprintf(__('You are running Simple SSL pro. A dedicated add-on for multisite has been released. If you want more options to have full control over your multisite network, you can ask for a discount code to %supgrade%s your license to a multisite license.', 'simple-ssl'), '<a href="https://simple-ssl.com/contact" title="Simple SSL">', '</a>') ?>
                            </p>
                            </p></div>
                        <?php
                    }
                    ...
                    <div id="message" class="updated notice is-dismissible rsssl-pro-dismiss-notice">
                        <p>
                            <?php echo sprintf(__('If you want more options to have full control over your multisite network, you can %supgrade%s your license to a multisite license, or dismiss this message', 'simple-ssl'), '<a href="https://simple-ssl.com/pro-multisite" title="Simple SSL">', '</a>') ?>
                        </p>
                        </p></div>
                    <?php
                }
        ...
        {
            return (isset($_GET['page']) && $_GET['page'] == 'simple-ssl') ? true : false;
        }
        ...
        public function admin_tabs($current = 'settings')
        {
            ...
            if (count($tabs) > 1) {
                ...
                foreach ($tabs as $tab => $name) {
                    ...
                    echo "<a class='nav-tab$class' href='?page=simple-ssl&tab=$tab'>$name</a>";
                }
                echo '</h2>';
            }
        }
