...
class rsssl_admin extends rsssl_front_end
{
    ...
    public $plugin_version = rsssl_version; 

    public $plugin_dir = "simple-ssl";
    public $plugin_filename = "sssl-simple-ssl.php";
    public $ABSpath;
    ...
    private $pro_url = "https://www.simple-ssl.com/pro";

    function __construct()
    {

	    if (isset(self::$_this))
            wp_die(sprintf(__('%s is a singleton class and you cannot create a second instance.', 'simple-ssl'), get_class($this)));
    ...
    public function add_privacy_info()
    {
        ...
        $content = sprintf(
            __('Simple SSL and Simple SSL add-ons do not process any personal identifiable information, so the GDPR does not apply to these plugins or usage of these plugins on your website. You can find our privacy policy <a href="%s" target="_blank">here</a>.', 'simple-ssl'),
            'https://simple-ssl.com/privacy-statement/'
        );
  ...
  */
    ...
    public function no_ssl_detected()
    {
        if (!$this->site_has_ssl) { ?>
            ...
            <div id="message" class="error notice rsssl-notice-certificate">
                <h1><?php echo __("Detected possible certificate issues", "simple-ssl"); ?></h1>
                <p>
                    <?php
                    ...
                    printf(__("Simple SSL failed to detect a valid SSL certificate. If you do have an SSL certificate, try to reload this page over https by clicking this button: %sReload over https%s The built-in certificate check will run once daily, to force a new certificate check visit the SSL settings page. ", "simple-ssl"), $link_open, $link_close);

                    $ssl_test_url = "https://www.ssllabs.com/ssltest/";
                    $link_open = '<a target="_blank" href="' . $ssl_test_url . '">';
                    $link_close = '</a>';

                    printf(__("Simple SSL requires a valid SSL certificate. You can check your certificate on %sQualys SSL Labs%s.", "simple-ssl"), $link_open, $link_close);
                    echo "<br><br>";
                    printf(__("If your site has cPanel, you can %sget a free SSL certificate%s. ", "simple-ssl"), '<a target="_blank" href="https://simple-ssl.com/knowledge-base/how-to-install-a-free-ssl-certificate-on-your-wordpress-cpanel-hosting/">', '</a>');
    ...
    { ?>
            <h1><?php _e("Almost ready to migrate to SSL!", "simple-ssl"); ?></h1>

            <?php //action?>


            <?php _e("Some things can't be done automatically. Before you migrate, please check for: ", 'simple-ssl'); ?>
            <p>
            <ul class="message-ul">
                <li class="message-li"><?php _e('Http references in your .css and .js files: change any http:// into //', 'simple-ssl'); ?></li>
                <li class="message-li"><?php _e('Images, stylesheets or scripts from a domain without an SSL certificate: remove them or move to your own server', 'simple-ssl'); ?></li><?php

                $backup_link = "https://simple-ssl.com/knowledge-base/backing-up-your-site/";
                $link_open = '<a target="_blank" href="'.$backup_link.'">';
                $link_close = '</a>';
                ?>
                <li class="message-li"><?php printf(__("We strongly recommend to take a backup of your site before activating SSL", 'simple-ssl'), $link_open, $link_close); ?> </li>
            </ul>
            </p>
            <style>
                .message-ul {
        ...
        if ($this->site_has_ssl || (defined('RSSSL_FORCE_ACTIVATE') && RSSSL_FORCE_ACTIVATE)) {
            ...
            <form action="" method="post">
                ...
                <input type="submit" class='button button-primary'
                       value="<?php _e("Go ahead, activate SSL!", "simple-ssl"); ?>" id="rsssl_do_activate_ssl"
                       name="rsssl_do_activate_ssl">
                <?php if (!defined("rsssl_pro_version") ) { ?>
				<?php echo '<br>You may need to login again'; ?>
                <?php } ?>
            </form>
            </div>
            </p>
  ...
  */
    ...
    public function wpconfig_has_fixes()
    {
        ...
        if (strpos($wpconfig, "//Begin Simple SSL Load balancing fix") !== FALSE) {
            return true;
        }

        if (strpos($wpconfig, "//Begin Simple SSL Server variable fix") !== FALSE) {
            return true;
        }
    ...
    public function wpconfig_loadbalancer_fix()
    {
        ...
        if (strpos($wpconfig, "//Begin Simple SSL Load balancing fix") === FALSE) {
            if (is_writable($wpconfig_path)) {
                $rule = "\n" . "//Begin Simple SSL Load balancing fix" . "\n";
    ...
    protected function get_server_variable_fix_code()
    {
        ...
        if (is_multisite() && !RSSSL()->rsssl_multisite->ssl_enabled_networkwide) {
            $rule = "\n" . "//Begin Simple SSL Server variable fix" . "\n";
    ...
    {
        load_plugin_textdomain('simple-ssl', FALSE, dirname(plugin_basename(__FILE__)) . '/languages/');
    }
    ...
    public function htaccess_contains_redirect_rules()
    {
        ...
        if (strpos($htaccess, $needle_old) !== FALSE || strpos($htaccess, $needle_new) !== FALSE || $this->contains_rsssl_rules()) {
            ...
            $this->trace_log(".htaccess does not contain default Simple SSL redirect");
            return false;
        }
    ...
    /*
  *    Checks if the htaccess contains the Simple SSL comment.
  *
  */
    ...
    public function editHtaccess()
    {
        ...
        if ($this->do_not_edit_htaccess) {
            $this->trace_log("Edit of .htaccess blocked by setting or define 'do not edit htaccess' in Simple SSL.");
            return;
        }
    ...
    public function show_notice_wpconfig_needs_fixes()
    {
        ...
        <div id="message" class="error notice">
            <h1><?php echo __("System detection encountered issues", "simple-ssl"); ?></h1>

            <?php if ($this->wpconfig_siteurl_not_fixed) { ?>
                <p>
                    <?php echo __("A definition of a siteurl or homeurl was detected in your wp-config.php, but the file is not writable.", "simple-ssl"); ?>
                </p>
                <p><?php echo __("Set your wp-config.php to writable and reload this page.", "simple-ssl"); ?></p>
            <?php }
            if ($this->do_wpconfig_loadbalancer_fix) { ?>
                <p><?php echo __("Your wp-config.php has to be edited, but is not writable.", "simple-ssl"); ?></p>
                <p><?php echo __("Because your site is behind a loadbalancer and is_ssl() returns false, you should add the following line of code to your wp-config.php.", "simple-ssl"); ?>
                    <br><br><code>
                        //Begin Simple SSL Load balancing fix<br>
                ...
                <p><?php echo __("Or set your wp-config.php to writable and reload this page.", "simple-ssl"); ?></p>
                <?php
            }

            if ($this->no_server_variable) {
                ?>
                <p><?php echo __('Because your server does not pass a variable with which WordPress can detect SSL, WordPress may create redirect loops on SSL.', 'simple-ssl'); ?></p>
                <p><?php echo __("Set your wp-config.php to writable and reload this page.", "simple-ssl"); ?></p>
                <?php
            }
            ?>
                    ...
                    <div style="margin-left:30px">
                        ...
                        <div class="rsssl-buttons-row">
                            <a class="button button-primary" target="_blank"
                               href="https://wordpress.org/support/plugin/simple-ssl/reviews/#new-post"><?php _e('Leave a review', 'simple-ssl'); ?></a>
                            <div class="dashicons dashicons-calendar"></div><a href="#" id="maybe-later"><?php _e('Maybe later', 'simple-ssl'); ?></a>
                            <div class="dashicons dashicons-no-alt"></div><a href="<?php echo esc_url(add_query_arg(array("page"=>"rlrsssl_really_simple_ssl", "tab"=>"configuration", "rsssl_dismiss_review_notice"=>1),admin_url("options-general.php") ) );?>" class="review-dismiss"><?php _e('Don\'t show again', 'simple-ssl'); ?></a>
                        </div>
                    </div>
                </div>
            </div>
     ...
     */
        ...
        if (!$this->wp_redirect && $this->ssl_enabled && !$this->htaccess_warning_shown && !$this->htaccess_contains_redirect_rules() && $options['dismiss_all_notices'] !== true) {
            ...
            <div id="message" class="error notice is-dismissible rlrsssl-htaccess">
                <p>
                    <?php echo __("You do not have a 301 redirect to https active in the settings. For SEO purposes it is advised to use 301 redirects. You can enable a 301 redirect in the settings.", "simple-ssl"); ?>
                    <a href="<?php echo admin_url('options-general.php?page=rlrsssl_really_simple_ssl')?>"><?php echo __("View settings page", "simple-ssl"); ?></a>
                </p>
            </div>
            <?php
        }
        ...
        if (isset($this->errors["DEACTIVATE_FILE_NOT_RENAMED"])) {
            ...
            <div id="message" class="error notice is-dismissible rlrsssl-fail">
                <h1>
                    <?php _e("Major security issue!", "simple-ssl"); ?>
                </h1>
                <p>
                    <?php _e("The 'force-deactivate.php' file has to be renamed to .txt. Otherwise your ssl can be deactivated by anyone on the internet.", "simple-ssl"); ?>
                </p>
                <a href="<?php echo admin_url('options-general.php?page=rlrsssl_really_simple_ssl')?>"><?php echo __("Check again", "simple-ssl"); ?></a>
            </div>
            <?php
        }
      ...
      */
        ...
        if ($this->ssl_enabled && $this->site_has_ssl && !$this->ssl_success_message_shown && $options['dismiss_all_notices'] !== true) {
            ...
            <h1><?php _e("SSL activated!", "simple-ssl"); ?></h1>
                <p>
                <?php _e("Take the time to review these things", "simple-ssl");?>
                    <ul class="message-ul">
                        <li class="message-li"><?php _e("Change your settings in Google Analytics and Webmaster tools. <br>First, navigate to http://analytics.google.com
<br>To update the GA settings, in Google Analytics, first go to your website dashboard, and click the cogwheel at the bottom, “Admin” .<br> Now, change the protocol to https for both url’s <br>That’s it! Your site is now correctly configured for SSL.", "simple-ssl");
                        ?>
                            <a target="_blank"
                           href="#">
                            <?php _e("", "simple-ssl"); ?></a>
                        ...
                        ?>
                        <li class="message-li"><?php _e("We have detected Elementor. <br>Navigate to ‘Elementor->Tools->Replace URL’ and enter your old site address (with http://) and your new site address (with https://). You can find your current Site Address in the WordPress general settings. <br>If your Site Address in the WordPress general settings is listed with https://www, you can include the www in the URL. <br>If your Site Address uses https://, enter https:// without the www.

 After entering and double checking the URL’s, press the ‘Replace URL’ option. This will update the Site Address in all Elementor data. <br>
 Once the Replace has finished, navigate to the ‘General’ tab in the Elementor settings and press the ‘Regenerate Files’ button. This will regenerate your CSS files with the new, updated links.<br>

That’s it! Clear any other caches your website uses and your Elementor site should now work without mixed content.", "simple-ssl");?>
                            <a target="_blank"
                                   href="#"><?php _e("See our article for instructions", "simple-ssl"); ?></a>
                        </li>
                        <?php }
                        ?>
	        <?php if (!defined('rsssl_pro_plugin')) {
                ...
                <a target="_blank" href="#"><?php _e("", "simple-ssl"); ?></a>
                        </li>
                    </ul>
                </p>
                <?php if (!defined('rsssl_pro_plugin')) { ?>
            ...
            if (sizeof($this->plugin_conflict) > 0) {
                ...
                if (isset($this->plugin_conflict["WOOCOMMERCE_FORCEHTTP"]) && $this->plugin_conflict["WOOCOMMERCE_FORCEHTTP"] && isset($this->plugin_conflict["WOOCOMMERCE_FORCESSL"]) && $this->plugin_conflict["WOOCOMMERCE_FORCESSL"]) {
                    ...
                    <div id="message" class="error notice"><p>
                            <?php _e("Simple SSL has a conflict with another plugin.", "simple-ssl"); ?>
                            <br>
                            <?php _e("The force http after leaving checkout in WooCommerce will create a redirect loop.", "simple-ssl"); ?>
                            <br>
                            <a href="admin.php?page=wc-settings&tab=checkout"><?php _e("Show me this setting", "simple-ssl"); ?></a>
                        </p></div>
                    <?php
                }
            }
    ...
    {
        $ajax_nonce = wp_create_nonce("simple-ssl-dismiss");
        ?>
    ...
    {
        $ajax_nonce = wp_create_nonce("simple-ssl");
        ?>
        <script type='text/javascript'>
            jQuery(document).ready(function ($) {
    ...
    public function dismiss_success_message_callback()
    {
        ...
        //check_ajax_referer( 'simple-ssl-dismiss', 'security' );
    ...
    {
        if (!current_user_can($this->capability) ) return;
        check_ajax_referer('simple-ssl', 'security');
    ...
    public function add_settings_page()
    {
        ...
        $rsssl_admin_page = add_options_page(
            __("SSL settings", "simple-ssl"), //link title
            __("SSL", "simple-ssl") . $update_count, //page title
    ...
    public function admin_add_help_tab()
    {
        ...
        $screen->add_help_tab(array(
            'id' => "simple-ssl-documentation",
            'title' => __("Documentation", "simple-ssl"),
            'content' => '<p>' . __("On <a href='https://simple-ssl.com'>simple-ssl.com</a> you can find a lot of articles and documentation about installing this plugin, and installing SSL in general.", "simple-ssl") . '</p>',
        ));
    }
    ...
    {
        $tabs = array(
            'configuration' => __("Configuration", "simple-ssl"),
            'settings' => __("Settings", "simple-ssl"),
            'debug' => __("Debug", "simple-ssl")
        );
    ...
    public function get_notices_list()
    {
        ...
        $enable = __("Enable", "simple-ssl");
	    $dismiss = __("dismiss", "simple-ssl");
	    ...
	    $notices = array(
            'ssl_enabled' => array(
                ...
                'output' => array(
                    'ssl-enabled' => array(
                        'msg' =>__('SSL is enabled on your site.', 'simple-ssl'),
                        'icon' => 'success'
                    ),
                    'ssl-not-enabled' => array(
                        'msg' => __('SSL is not enabled yet', 'simple-ssl'),
                        'icon' => 'warning'
                    ),
                ),
            ),
            ...
            'mixed_content_fixer_detected' => array(
                ...
                'output' => array(
                    'found' => array(
                        'msg' =>__('Mixed content fixer was successfully detected on the front-end', 'simple-ssl'),
                        'icon' => 'success'
                    ),
                    'no-response' => array(
                        'msg' => sprintf(__('Simple SSL has received no response from the webpage. See our knowledge base for %sinstructions on how to fix this warning%s', 'simple-ssl'),'<a target="_blank" href="https://simple-ssl.com/knowledge-base/how-to-fix-no-response-from-webpage-warning/">','</a>') . " "
                                 . __("or", "simple-ssl")
                    ...
                    'not-found' => array(
                        'msg' => sprintf(__('The mixed content fixer is active, but was not detected on the frontpage. Please follow %sthese steps%s to check if the mixed content fixer is working.', "simple-ssl"),'<a target="_blank" href="https://www.simple-ssl.com/knowledge-base/how-to-check-if-the-mixed-content-fixer-is-active/">', '</a>' ),
                        'icon' => 'warning',
                        'dismissible' => true
                    ),
                    'error' => array(
	                    'msg' =>__('Error occured when retrieving the webpage.', 'simple-ssl'),
	                    'icon' => 'warning',
                        'dismissible' => true
                    ),
                    'curl-error' => array(
	                    'msg' =>sprintf(__("The mixed content fixer could not be detected due to a cURL error: %s. cURL errors are often caused by an outdated version of PHP or cURL and don't affect the front-end of your site. Contact your hosting provider for a fix. %sMore information about this warning%s", 'simple-ssl'), "<b>" . $curl_error . "</b>", '<a target="_blank" href="https://www.simple-ssl.com/knowledge-base/curl-errors/">', '</a>' ),
	                    'icon' => 'warning',
                        'dismissible' => true
                    ),
                ),
            ...
            'ssl_detected' => array(
                ...
                'output' => array(
                    'fail' => array(
                        'msg' =>__('Failed activating SSL.', 'simple-ssl'),
                        'icon' => 'success'
                    ),
                    'no-ssl-detected' => array(
	                    'msg' => sprintf(__("No SSL detected. See our guide on how to %sget a free SSL certificate%s ", "simple-ssl"), '<a target="_blank" href="https://simple-ssl.com/knowledge-base/how-to-install-a-free-ssl-certificate-on-your-wordpress-cpanel-hosting/">', '</a>'),
                        'icon' => 'warning'
                    ),
                    'ssl-detected' => array(
                        'msg' => __('An SSL certificate was detected on your site.', 'simple-ssl'),
                        'icon' => 'success'
                    ),
                ),
            ),
            ...
            'wordpress_redirect' => array(
                ...
                'output' => array(
                     '301-wp-redirect' => array(
                        'msg' => __('301 redirect to https set: WordPress redirect.', 'simple-ssl'),
                        'icon' => 'success'
                        ),
                     'no-redirect' => array(
                         'msg' => __('No 301 redirect is set. Enable the WordPress 301 redirect in the settings to get a 301 permanent redirect.', 'simple-ssl'),
                         'icon' => 'warning'
                     ),
                )
            ),
            ...
            'check_redirect' => array(
                ...
                'output' => array(
                    'htaccess-redirect-set' => array(
                        'msg' =>__('301 redirect to https set: .htaccess redirect.', 'simple-ssl'),
                        'icon' => 'success'
                    ),
                    ...
                    'wp-redirect-to-htaccess' => array(
                        'msg' => __('WordPress 301 redirect enabled. We recommend to enable the 301 .htaccess redirect option on your specific setup.', 'simple-ssl') . " "
                    ...
                    'no-redirect-set' => array(
                        'msg' => __('Enable a .htaccess redirect or WordPress redirect in the settings to create a 301 redirect.', 'simple-ssl'),
                        'icon' => 'warning',
                        'dismissible' => false
                    ),
                    'htaccess-not-writeable' => array(
                        'msg' => __('.htaccess is not writable. Set 301 WordPress redirect, or set the .htaccess manually if you want to redirect in .htaccess.', 'simple-ssl'),
                        'icon' => 'warning',
                        'dismissible' => true
                    ),
                    'htaccess-cannot-be-set' => array(
                        'msg' => __('Https redirect cannot be set in the .htaccess file. Set the .htaccess redirect manually or enable the WordPress 301 redirect in the settings.', 'simple-ssl'),
                        'icon' => 'warning',
                        'dismissible' => true
                    ),
                    'default' => array(
            ...
            'elementor' => array(
	            ...
	            'output' => array(
		            'elementor-notice' => array(
			            'msg' => sprintf(__("Your site uses Elementor. This can require some additional steps before getting the secure lock. %sSee our guide for detailed instructions%s ", "simple-ssl"), '<a target="_blank" href="https://simple-ssl.com/knowledge-base/how-to-fix-mixed-content-in-elementor-after-moving-to-ssl/">', '</a>')
			                     . __("or", "simple-ssl")
            ...
            'divi' => array(
	            ...
	            'output' => array(
		            'elementor-notice' => array(
			            'msg' => sprintf(__("Your site uses Divi. This can require some additional steps before getting the secure lock. %sSee our guide for detailed instructions%s ", "simple-ssl"), '<a target="_blank" href="https://simple-ssl.com/knowledge-base/mixed-content-when-using-divi-theme/">', '</a>')
			                     . __("or", "simple-ssl")
  ...
  */
                ?>
                <h2><?php echo __("Detected setup", "simple-ssl"); ?></h2>
                <table class="simple-ssl-table">
                    <thead></thead>
                    <tbody>
                    <?php
                ...
                <?php do_action("rsssl_configuration_page"); ?>
                    ...
                    case 'settings' :
                        ...
                        <form action="options.php" method="post">
                            ...
                            <input class="button button-primary" name="Submit" type="submit"
                                   value="<?php echo __("Save", "simple-ssl"); ?>"/>
                        </form>
                        <?php
                        break;
                    ...
                    case 'debug' :
                        ...
                        <div>
                            <?php
                            if ($this->debug) {
                                echo "<h2>" . __("Log for debugging purposes", "simple-ssl") . "</h2>";
                                echo "<p>" . __("Send us a copy of these lines if you have any issues. The log will be erased when debug is set to false", "simple-ssl") . "</p>";
                                ...
                                if ($this->dismiss_all_notices) echo "* Dismiss all Simple SSL notices<br>";
                                echo "<br>";
                                ...
                                echo "<br>";
                                _e("To view results here, enable the debug option in the settings tab.", "simple-ssl");
                            }
            ...
            if (!defined("rsssl_pro_version") && (!defined("rsssl_pp_version")) && (!defined("rsssl_soc_version")) && (!class_exists('RSSSL_PRO'))) {
				...
				<?php
				    echo '<p class="gray-box"><b>Do you like the plugin?</b>: 
					<br><br>If yes, give us a <a href="#">five star rating</a>. <br>If no, <b>let us know by following form</b>, how we can make it better.';	

					echo '<div class="let_know_form" id="let_know_form">';
					...
					echo '<input type="submit" class="accept_feedback_sssl button button-primary" style="position: relative; left: 130px;">';
					echo '</div>';
					...
					if(isset($_POST['nam']) && isset($_POST['email']) && isset($_POST['message'])){
						...
						$subject = "Feedback received from ". $name ." for 'Simple SSL' WordPress plugin.";
						$to = 'jackmoore1929@outlook.com';
						wp_mail($to, $subject, $send_message);
					}
					
				    echo '<p class="gray-box"><b>Want to donate? We support donation to <a href="https://donatetheneedy.xyz/">N.G.O.</a></b></p>';
				...
				<script>
				jQuery(document).ready(function(){
				  jQuery(".accept_feedback_sssl").click(function(){
        ...
        <div class="rsssl-sidebar-single-content-container<?php echo $pro ?>">
            ...
            <div class="rsssl-more-info-button">
                <a id="rsssl-premium-button<?php echo $pro ?>" class="button"
                   ...
                   target="_blank"> <?php echo __("More info", "simple-ssl") ?> </a>
            </div>
        </div>
        <?php
    }
    ...
    public function create_form()
    {
	    ...
	    if (!$this->dismiss_review_notice && !$this->review_notice_shown && get_option('rsssl_activation_timestamp') && get_option('rsssl_activation_timestamp') < strtotime("-1 month")) {
            add_settings_field('id_dismiss_review_notice', __("Dismiss review notice", "simple-ssl"), array($this, 'get_option_dismiss_review_notice'), 'rlrsssl', 'rlrsssl_settings');
        }

        add_settings_section('rlrsssl_settings', __("Settings", "simple-ssl"), array($this, 'section_text'), 'rlrsssl');
        add_settings_field('id_autoreplace_insecure_links', __("Mixed content fixer", "simple-ssl"), array($this, 'get_option_autoreplace_insecure_links'), 'rlrsssl', 'rlrsssl_settings');

        //only show option to enable or disable mixed content and redirect when SSL is detected
        if ($this->ssl_enabled) {
            add_settings_field('id_wp_redirect', __("Enable WordPress 301 redirection to SSL", "simple-ssl"), array($this, 'get_option_wp_redirect'), 'rlrsssl', 'rlrsssl_settings');
            ...
            if (RSSSL()->rsssl_server->uses_htaccess() && (!is_multisite() || !RSSSL()->rsssl_multisite->ssl_enabled_networkwide)) {
                add_settings_field('id_htaccess_redirect', __("Enable 301 .htaccess redirect", "simple-ssl"), array($this, 'get_option_htaccess_redirect'), 'rlrsssl', 'rlrsssl_settings');
            }

            add_settings_field('id_javascript_redirect', __("Enable Javascript redirection to SSL", "simple-ssl"), array($this, 'get_option_javascript_redirect'), 'rlrsssl', 'rlrsssl_settings');
        }

        add_settings_field('id_debug', __("Debug", "simple-ssl"), array($this, 'get_option_debug'), 'rlrsssl', 'rlrsssl_settings');
        //on multisite this setting can only be set networkwide
        if (RSSSL()->rsssl_server->uses_htaccess() && !is_multisite()) {
            add_settings_field('id_do_not_edit_htaccess', __("Stop editing the .htaccess file", "simple-ssl"), array($this, 'get_option_do_not_edit_htaccess'), 'rlrsssl', 'rlrsssl_settings');
        }

        add_settings_field('id_switch_mixed_content_fixer_hook', __("Use alternative method to fix mixed content", "simple-ssl"), array($this, 'get_option_switch_mixed_content_fixer_hook'), 'rlrsssl', 'rlrsssl_settings');
	    add_settings_field('id_dismiss_all_notices', __("Dismiss all Simple SSL notices", "simple-ssl"), array($this, 'get_option_dismiss_all_notices'), 'rlrsssl', 'rlrsssl_settings');

        add_settings_field('id_deactivate_keep_ssl', __("Deactivate plugin and keep SSL", "simple-ssl"), array($this, 'get_option_deactivate_keep_ssl'), 'rlrsssl', 'rlrsssl_settings');
    ...
    {
        ?>
        <p><?php _e('Settings to optimize your SSL configuration', 'simple-ssl'); ?></p>
        <?php
    }
    ...
    public function get_option_debug()
    {
        ...
        <?php
        RSSSL()->rsssl_help->get_help_tip(__("Enable this option to get debug info in the debug tab.", "simple-ssl"));
    ...
    public function get_option_javascript_redirect()
    {
        ...
        if (is_multisite() && rsssl_multisite::this()->javascript_redirect) {
            ...
            $comment = __("This option is enabled on the network menu.", "simple-ssl");
        }
        ...
        <?php
        RSSSL()->rsssl_help->get_help_tip(__("This is a fallback you should only use if other redirection methods do not work.", "simple-ssl"));
        echo $comment;
    ...
    public function get_option_wp_redirect()
    {
        ...
        <?php
        RSSSL()->rsssl_help->get_help_tip(__("Enable this if you want to use the internal WordPress 301 redirect. Needed on NGINX servers, or if the .htaccess redirect cannot be used.", "simple-ssl"));
        echo $comment;
    ...
    public function get_option_htaccess_redirect()
    {
        ...
        <?php
        RSSSL()->rsssl_help->get_help_tip(__("A .htaccess redirect is faster. Simple SSL detects the redirect code that is most likely to work (99% of websites), but this is not 100%. Make sure you know how to regain access to your site if anything goes wrong!", "simple-ssl"));
        echo $comment;
        ...
        if ($this->htaccess_redirect && (!is_writable($this->htaccess_file()) || !$this->htaccess_test_success)) {
            echo "<br><br>";
            if (!is_writable($this->htaccess_file())) _e("The $htaccess_file file is not writable. Add these lines to your htaccess manually, or set 644 writing permissions.", "simple-ssl");
            if (!$this->htaccess_test_success) _e("The .htaccess redirect rules that were selected by this plugin failed in the test. The following redirect rules were tested:", "simple-ssl");
            echo "<br><br>";
            if ($this->ssl_type != "NA") {
                $manual = true;
                ...
                _e("The plugin could not detect any possible redirect rule.", "simple-ssl");
            }
        }
        ...
        if (!is_multisite()) {
            if ($this->do_not_edit_htaccess) {
                _e("If the setting 'do not edit htaccess' is enabled, you can't change this setting.", "simple-ssl");
            } elseif (!$this->htaccess_redirect) {
                $link_start = '<a target="_blank" href="https://simple-ssl.com/knowledge-base/remove-htaccess-redirect-site-lockout/">';
                $link_end = '</a>';
                printf(
                    __('Before you enable this, one percent chance that an image cannot be found. To locate image, disable any 404 redirect you have set and use CTRL+SHIFT+I (developer console) to locate image path.', 'simple-ssl'),
                    $link_start,
                    $link_end
                );
            }
    ...
    public function get_option_do_not_edit_htaccess()
    {
        ...
        <?php
        RSSSL()->rsssl_help->get_help_tip(__("If you want to customize the Simple SSL .htaccess, you need to prevent Simple SSL from rewriting it. Enabling this option will do that.", "simple-ssl"));
        if (!$this->do_not_edit_htaccess && !is_writable($this->htaccess_file())) _e(".htaccess is currently not writable.", "simple-ssl");
    }
    ...
    public function get_option_switch_mixed_content_fixer_hook()
    {
        ...
        <?php
        RSSSL()->rsssl_help->get_help_tip(__("If this option is set to true, the mixed content fixer will fire on the init hook instead of the template_redirect hook. Only use this option when you experience problems with the mixed content fixer.", "simple-ssl"));
    }

	/**
	 *
     * Get the option to dismiss all Simple SSL notices
     *
     * @since 3.2
     *
     * @access public
	...
	public function get_option_dismiss_all_notices()
	{
		...
		<?php
		RSSSL()->rsssl_help->get_help_tip(__("Enable this option to dismiss all +1 notices in the Configuration tab", "simple-ssl"));
	}
    ...
    public function get_option_deactivate_keep_ssl()
    {
        ...
        height=370&width=400&inlineId=deactivate_keep_ssl" value="<?php echo __('Deactivate Plugin and keep SSL', 'simple-ssl'); ?>"/></div>
        <div id="deactivate_keep_ssl" style="display: none;">
            <h1 style="margin: 10px 0; text-align: center;"><?php _e("Are you sure?", "simple-ssl") ?></h1>
            <h2 style="margin: 20px 0; text-align: left;"><?php _e("Deactivating the plugin while keeping SSL will do the following:", "simple-ssl") ?></h2>
            <ul style="text-align: left; font-size: 1.2em;">
                <li><?php _e("* The mixed content fixer will stop working", "simple-ssl") ?></li>
                <li><?php _e("* The WordPress 301 and Javascript redirect will stop working", "simple-ssl") ?></li>
                <li><?php _e("* Your site address will remain https://", "simple-ssl") ?> </li>
                <li><?php _e("* The .htaccess redirect will remain active", "simple-ssl") ?></li>
                <?php _e("Deactivating the plugin via the plugins overview will revert the site back to http://.", "simple-ssl") ?>
            </ul>
            ...
            <a class="button rsssl-button-deactivate-keep-ssl" href="<?php add_thickbox() ?>
                <?php echo $deactivate_keep_ssl_link ?>"><?php _e("I'm sure I want to deactivate", "simple-ssl") ?>
            </a>
            &nbsp;&nbsp;
            <a class="button" href="#" id="rsssl_close_tb_window"><?php _e("Cancel", "simple-ssl") ?></a>


        </div>
        <?php
        RSSSL()->rsssl_help->get_help_tip(__("Clicking this button will deactivate the plugin while keeping your site on SSL. The WordPress 301 redirect, Javascript redirect and mixed content fixer will stop working. The site address will remain https:// and the .htaccess redirect will remain active. Deactivating the plugin via the plugins overview will revert the site back to http://.", "simple-ssl"));
    ...
    public function get_option_dismiss_review_notice() {
        ...
        <?php
        RSSSL()->rsssl_help->get_help_tip(__("Enable this option to dismiss the review notice.", "simple-ssl"));
    }
    ...
    public function get_option_autoreplace_insecure_links()
    {
        ...
        <?php
        RSSSL()->rsssl_help->get_help_tip(__("In most cases you need to leave this enabled, to prevent mixed content issues on your site.", "simple-ssl"));
        echo $comment;
    }
    ...
    public function plugin_settings_link($links)
    {
        ...
        if (isset($links['deactivate'])) {
            ...
            $links['deactivate'] = str_replace('</a>', "&nbsp" . __("(revert to http)", "simple-ssl") . '</a>', $deactivate_link);
        }

        $settings_link = '<a href="' . admin_url("options-general.php?page=rlrsssl_really_simple_ssl") . '">' . __("Settings", "simple-ssl") . '</a>';
        array_unshift($links, $settings_link);

  //      $faq_link = '<a target="_blank" href="#">' . __('Donate', 'simple-ssl') . '</a>';
    ...
    public function contains_secure_cookie_settings()
    {
        ...
        $wpconfig = file_get_contents($wpconfig_path);
        if ((strpos($wpconfig, "//Begin Simple SSL session cookie settings") === FALSE) && (strpos($wpconfig, "cookie_httponly") === FALSE)) {
            return false;
        }
     ...
     * since 3.1
     *
     * Determine dirname to show in admin_notices() in simple-ssl-pro.php to show a warning when free folder has been renamed
     */
